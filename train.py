...
def set_up_training_loops(log, args, path_data, trainset_name, path_models, train_config, model_config, net, num_gpus, device):
    ...
    for idx, crop_size in enumerate(args.crop_size): # Cicle over multiple 'crop_size' if provided
        ...
        for i in range(args.iterations): # Train multiple models
            ...
            if args.retrain: # TODO: To change/optimize this sub-condition. 
                ...
                if get_file(old_model.parent / "{}.json".format(old_model.stem))['architecture'][-1] != train_config['architecture'][-1]:
                ...
                train_config['retrain_model'] = old_model.stem

            # Pre-training of the Encoder in autoencoder style
            train_config['pre_trained'] = False
            if args.pre_train:
                ...
                train_auto(net=net_auto, dataset=datasets, configs=train_config, device=device,  path_models=path_models)
                ...
                train_config['pre_trained'] = True
                del net_auto
            
            # Load training and validation set - this is the train (or fine-tuning after the pre-training phase).
            data_transforms = augmentors(label_type = train_config['label_type'], min_value=0, max_value=65535)
            train_config['data_transforms'] = str(data_transforms)
            ...
            train_config['max_epochs'] = get_max_epochs(len(datasets['train']) + len(datasets['val']))
            # NOTE: Make the training.py more clean - all computation like the one belowe are passed from the calling function
            print(f"Number of epochs without improvement allowed {2 * train_config['max_epochs'] // 20 + 5}")
            # Train loop with the chosen architecture.
            best_loss = train(log=log, net=net, datasets=datasets, configs=train_config, device=device, path_models=path_models)

             # Fine-tune with cosine annealing for Ranger models - taken the current best model just trained.
            if train_config['optimizer'] == 'ranger':
                ...
                # Train further
                _ = train(net=net, datasets=datasets, configs=train_config, device=device, path_models=path_models, best_loss=best_loss)

            # Write information to json-file
            write_train_info(configs=train_config, path=path_models)
    return
...
def set_up_training():
    ...
    train_config, model_config = parse_training_args(log, args, num_gpus)
    net = create_model_architecture(log, args.pre_train, model_config, device, num_gpus)
    set_up_training_loops(log, args, path_data, trainset_name, path_models, train_config, model_config, net, num_gpus, device)
